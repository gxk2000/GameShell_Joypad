...
unsigned long Ntime;
unsigned long Ntime_set;
int Ntime_flag = KEY_ON;
int Ntime_count = 0;
...
void rest_led(int rest_led_stat) {
  if (rest_led_stat == KEY_ON) {
    if (Ntime_flag == KEY_ON) {
      Ntime_set = return_time();
      Ntime_flag = KEY_OFF;
    }

    if (return_time() - Ntime_set >= 800 and Ntime_count == 0) {
      Ntime_count++;
      Ntime_flag = KEY_ON;
    } else if (return_time() - Ntime_set >= 800 and Ntime_count == 1) {
      Ntime_count = 0;
      Ntime_flag = KEY_ON;
    }
...
void loop() {
  ...
  if (RestJudgment_Key1() == KEY_ON and RestJudgment_Key2() == KEY_ON) {
    ...
    if (rest_flag_1 == KEY_ON) {
      ...
      NowMillis = return_time();
      rest_flag_1 = KEY_OFF;
      rest_flag_2 = KEY_ON;
      //Serial.println("begin...");
    }
    // 比较时间戳并将原始x与y轴数据存入eeprom
    if (return_time() - NowMillis >= RestTime and rest_flag_2 == KEY_ON) {
