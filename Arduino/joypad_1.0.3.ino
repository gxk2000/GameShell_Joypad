...
void rest_led(int rest_led_stat) {
  if (rest_led_stat == KEY_ON) {
    if (LedStatusFlag == KEY_ON) {
      LedTimeStamp = return_time();
      LedStatusFlag = KEY_OFF;
    }

    if (return_time() - LedTimeStamp >= 800 and NowTimeCount == 0) {
      NowTimeCount++;
      LedStatusFlag = KEY_ON;
    } else if (return_time() - LedTimeStamp >= 800 and NowTimeCount == 1) {
      NowTimeCount = 0;
      LedStatusFlag = KEY_ON;
    }
...
void loop() {
  ...
  if (RestJudgment_Key1() == KEY_ON and RestJudgment_Key2() == KEY_ON) {
    ...
    if (rest_flag_1 == KEY_ON) {
      ...
      ResetTimeStamp = return_time();
      rest_flag_1 = KEY_OFF;
      rest_flag_2 = KEY_ON;
      //Serial.println("begin...");
    }
    // 比较时间戳并将原始x与y轴数据存入eeprom
    if (return_time() - ResetTimeStamp >= RestTime and rest_flag_2 == KEY_ON) {
